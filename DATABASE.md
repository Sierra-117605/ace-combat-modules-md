...
- ただし2026-06-27、Claude Code が GitHub dev版
  ...
  新規variant追加で実装できる構造であることを確認できたため、
...
- **MD既存実装との対応**: アーキタイプ自体は新規追加が必要(MD既存に該当なし、
  ...
  艦版に流用すれば構築可能。
- **備考**: シンファクシ級独自のモジュール候補は **バースト弾頭ミサイル**
  (対地・対空・対艦の三用途、種別④に追加済み)と
  **潜水艦用艦上滑走路**(V/STOL用、アリコーンと共有、種別④に追加済み)。
...
- **備考**: アリコーン独自のモジュール候補は **SRC-03a 超大型レールキャノン**
  (600mm、核砲弾)、**補助レールガン砲塔**(200mm×2基、80発/分、潜水艦専用)、
  **SLUAV 発射ベイ**(潜水艦から空中無人機を射出)、
  **潜水艦用艦上滑走路**(20-30機運用、シンファクシ級と共有)の4点。
  全件 MD既存と非競合のため種別④に追加済み。
  48基VLS は MD既存(`module_vls_category` / `module_vls_sub_category`)と
  直接競合のため独自モジュール化はせず、MDの既存モジュールを最大tierで
  割り当てる想定。
...
- **備考**: acecombat.wiki.ggではDisputed判定だが、本プロジェクトでは公式判定を優先。
  ...
  既存モジュール群の確認・必要に応じた調整に留まる見込み(具体的な
  調整要否は実装フェーズで個別判断、現時点では未確認)。
...
- **HOI4実装判定**: × 除外(機体本体)
- **判定根拠**: 機体本体追加は本MOD方針外。MQ-99 は「コンテナ射出後は独立飛行する
  単独完結UAV」であり、種別④で UAV モジュール化する案は 2026-06-28 検討の結果
  不可と判定済み(MDの `plane_droneswarm_weapon` は母機ステータス強化抽象化で、
  独立飛行する単独完結UAVを表現できないため。詳細は HANDOFF.md「群運用攻撃UAV
  (MQ-99系)を削除」エントリ参照)。
- **MD既存実装との対応**: 未確認(MD実コード未取得)
- **備考**: 本機自体はプレイヤー使用不可の敵UAV。実装するなら新規 UAV
  アーキタイプ追加が必要で、本MOD方針(機体本体追加は方針外)から外れる。
...
- **HOI4実装判定**: × 除外(機体本体)
- **判定根拠**: 機体本体追加は本MOD方針外。MQ-90 Quox は「独立飛行する単独完結
  UAV」であり、MQ-99 と同様に種別④モジュール化は不可と判定(MDの
  `plane_droneswarm_weapon` は母機ステータス強化抽象化で、独立飛行UAVを
  表現できない)。
- **MD既存実装との対応**: 未確認(MD実コード未取得)
- **備考**: ACI設定では制式採用UAV(2010年代に米国等へ輸出)。
  MQ-99(攻撃用)とは異なるW翼レイアウトが特徴。実装するなら新規 UAV
  アーキタイプ追加が必要で、本MOD方針外。
...
### 種別③ 由来の艦載モジュール候補(MD既存非競合のみ採用、5件)

| モジュール | 由来艦 | MD既存との関係 | 種別④ |
|------------|--------|----------------|--------|
| **SRC-03a 超大型レールキャノン**(600mm、核砲弾) | アリコーン | `module_railguns_category` の桁外れ上位tier | エントリ化済み |
| **補助レールガン砲塔**(200mm、80発/分、潜水艦専用) | アリコーン | 既存 railguns は水上艦専用スロット、潜水艦には未受入 → 潜水艦用新tier | エントリ化済み |
| **バースト弾頭ミサイル**(対地・対空・対艦) | シンファクシ級 | `module_vls_sub_category` 派生(burst warhead で三用途) | エントリ化済み |
| **潜水艦用艦上滑走路** | アリコーン / シンファクシ級 | 既存 `module_flight_decks_category` は水上艦専用、潜水艦には流用不可のため独自カテゴリ追加 | エントリ化済み |
| **SLUAV 発射ベイ**(潜水艦から空中UAVを射出) | アリコーン | `module_submarine_drone_control_category`(水中ドローン制御)とは別物のため新カテゴリで追加 | エントリ化済み |
...
| B | TLS(機載レーザ砲) | 2 | 特殊兵装枠(`plane_fighter_weapons`) | ADFX-01 → ADFX-02 |
| B | HPM(対エンジンマイクロ波) | 1 | 特殊兵装枠(`plane_fighter_weapons`) | XFA-33 Fenrir |
| C | 機載レールガン | 2 | 特殊兵装枠(`plane_fighter_weapons`) | CFA-44 EML → X-02S Arclight |
...
| G | 子機搭載ドローン群 | 2 | 特殊兵装枠(`plane_fighter_weapons`、母機強化として抽象化) | CFA-44 UAV-45s、ADF-11F 支援UAV |
...
| `plane_fighter_weapons`(本MOD新カテゴリ) | ACM-MD 独自追加 | TLS、HPM、機載レールガン、子機搭載ドローン群、MPBM、多目的全方位ミサイル、LSWM |
...
- **判定根拠(2026-06-28 本人再改訂)**: 本MOD独自カテゴリ
  `plane_fighter_weapons` に新モジュールとして追加し、通常戦闘機の
...
- **典拠URL**:
  ...
  - https://acecombat.wiki.gg/wiki/ADA-01B_ADLER
- **HOI4実装判定**: ○ モジュール化可
- **判定根拠**: 本MOD独自 `plane_fighter_weapons` カテゴリに新モジュール
...
- **MD既存実装との対応**: 数値参照元あり。
  ...
  (`weap_droneswarm_fighter_1` は air_attack +35 / air_agility +50)。
  本MODは通常戦闘機向けに下方修正した数値で `plane_fighter_weapons` カテゴリの
  新モジュールとして定義する。
...
- **設計方針(2026-06-28 本人確認)**: アーセナルバード専用カテゴリだった
  ...
  豪華設定のため、戦闘機ベースでは過剰となるため下方修正が必須。
...
#### バースト弾頭ミサイル (Burst Warhead Missile / 対地・対空・対艦)

- **種別**: モジュール構成要素(艦載)
- **登場作品**: AC5(シンファクシ・リムファクシ搭載)
- **製造主体(in-universe)**: ユークトバニア国営兵器工場
- **性質**: 大型潜水艦から発射される多用途バースト弾頭ミサイル。命中前に
  低空で炸裂し、爆発半径約1,500m(5,000ft)の拡散弾頭が広範囲を制圧する。
  **対空(航空編隊掃討)・対艦(空母2隻撃沈の前例あり)・対地(低高度
  起爆で「molten steel」が地上に降る)** の三用途すべてに有効。
- **典拠URL**:
  - https://acecombat.wiki.gg/wiki/Scinfaxi-class_submarine
  - https://acecombat.wiki.gg/wiki/Burst_missile
- **HOI4実装判定**: ○ モジュール化可
- **判定根拠**: MD既存の `module_vls_sub_category` (潜水艦VLS)に新モジュール
  として追加。MDの既存VLS_subは通常規模の対艦/対地ミサイル系で、
  burst warhead による「**広範囲三用途同時攻撃**」の性質は独自。
- **MD既存実装との対応**: 該当カテゴリあり。
  `module_vls_sub_category` 既存定義: `MD_ship_modules.txt:5256-5658` 付近に
  15tier。本MODのバースト弾頭ミサイルは既存通常VLS_subと差別化(
  anti-air 命中数ボーナス + naval_strike_attack + air_ground_attack の
  複合ボーナスを単一モジュールで提供)。
- **備考**: シンファクシ級独自のアイデンティティとなる**多用途戦略
  バースト兵器**。AC5本編では空母2隻撃沈(2010年9月30日)とサンドアイランド
  対地支援(同年10月4日)で使用された。
  通常VLSと同スロット競合のため、選択して載せる形になる。

---

#### 潜水艦用艦上滑走路 (Submarine-Mounted Flight Deck)

- **種別**: モジュール構成要素(艦載)
- **登場作品**: AC7(アリコーン搭載、20-30機の有人戦闘機運用)、
  AC5(シンファクシ級にも小型のV/STOL用艦尾離発着デッキあり)
- **製造主体(in-universe)**: エルジア(アリコーン)、ユークトバニア(シンファクシ級)
- **性質**: 大型潜水艦に組み込まれる艦上飛行甲板。水中潜航時は格納、浮上時に
  露出して固定翼機・V/STOL機を運用する。アリコーンは20-30機規模、
  シンファクシ級は艦尾の小型V/STOL用甲板。
- **典拠URL**:
  - https://acecombat.wiki.gg/wiki/Alicorn
  - https://acecombat.wiki.gg/wiki/Scinfaxi-class_submarine
- **HOI4実装判定**: ○ モジュール化可
- **判定根拠**: MD既存の `module_flight_decks_category` は水上艦
  (carrier / helicopter_operator)専用で、潜水艦 archetype
  (`attack_submarine`/`missile_submarine`、`MD_mtg_ships.txt:5424` および `:5914`)
  には flight_deck スロットが無い。本MODの「超大型潜水艦」アーキタイプ
  (新規追加)に新規カテゴリ `module_submarine_flight_deck_category`
  (仮称)+ 専用スロットを追加することで、既存実装と完全に非競合な独自
  モジュールとして実現可能。
- **MD既存実装との対応**: 直接対応カテゴリは無いが、設計参考として
  `module_flight_decks_category`(`MD_ship_modules.txt:1693-1959` 付近、
  14tier)が利用可。実装は新規カテゴリだが、ステータス効果(ship_carrier_size
  / 艦載機運用容量等)の付け方は既存flight_deckを参考にする。
- **備考**: 「潜水艦から固定翼機を運用する」は HOI4 のメカ上は珍しいが、
  ステータス効果(艦載機運用枠の付与)で表現可能。
  シンファクシ級・アリコーンの差別化に使う(アリコーン: 高tier、
  シンファクシ級: 低tier or V/STOL限定 tier)。

---

#### 補助レールガン砲塔 (Auxiliary Railgun Turret / 潜水艦専用)

- **種別**: モジュール構成要素(艦載)
- **登場作品**: AC7(アリコーン搭載、200mm口径×2基、80発/分)
- **製造主体(in-universe)**: ユークトバニア → エルジア(運用)
- **性質**: 大型潜水艦の上甲板に搭載される副砲級レールガン砲塔。
  200mm口径サボ弾、最大80発/分で連射可能。主砲(SRC-03a 600mm)と
  併用される副武装位置づけ。
- **典拠URL**:
  - https://acecombat.wiki.gg/wiki/Alicorn
- **HOI4実装判定**: ○ モジュール化可
- **判定根拠**: MD既存の `module_railguns_category` は水上艦専用で、
  既存潜水艦(`attack_submarine`/`missile_submarine`)のスロット定義
  (`MD_mtg_ships.txt:5439-5503` 他)には railgun 系カテゴリが一切無い。
  本MODの「超大型潜水艦」アーキタイプに railgun 受入スロットを追加し、
  そこに「潜水艦専用の副砲級 railgun」新tierを定義することで、既存
  水上艦実装と完全に非競合な独自モジュールとして実現可能。
- **MD既存実装との対応**: カテゴリ自体は既存(`module_railguns_category`、
  `MD_ship_modules.txt:4067-4286` 付近、6tier)。本MODは同カテゴリの
  新tier(潜水艦適合・連射特化)を追加 + 超大型潜水艦アーキタイプ専用
  スロットを介して受け入れる構造。
- **備考**: SRC-03a 超大型レールキャノン(主砲)とは別物。主砲枠とは別の
  「補助武装スロット」に積む想定。複数基(アリコーンは2基)搭載のため、
  HOI4 では1モジュールに2門ぶん相当の効果を持たせるか、複数スロット
  占有形式で表現するかは実装判断。
...
- **判定根拠**: MDには「潜水艦が空中UAVを射出する」モジュールカテゴリは
  既存しない(`module_submarine_drone_control_category` は水中ドローン制御で
  別物。モジュール名は `module_naval_submarine_drone_control`)。
  独立カテゴリ追加または既存 `module_submarine_drone_control_category` の
  派生として「空中UAV制御」を追加可能。航空機側の `plane_droneswarm_weapon`
  同様に「母艦の air_attack / air_defence / anti_air_attack を強化する
  モジュール」として抽象化実装する。
- **MD既存実装との対応**: 直接対応カテゴリは無いが、隣接カテゴリあり。
  `module_submarine_drone_control_category` 既存定義:
  `MD_ship_modules.txt:2939`(モジュール名 `module_naval_submarine_drone_control`)。
...
追加で実装できる構造であることを確認済み。「対応物あり」として記録。
...
- 種別別件数:
  ...
  - モジュール構成要素: 18(航空機13件 + 艦載5件)
  - 実在機: 0
- 判定別件数:
  - ○ モジュール化可: 18(航空機モジュール13件 + 艦載モジュール5件)
  - △ アーキタイプ追加: 9(超兵器セクションの空中艦・空母系)
  - × 除外: 24(超兵器固定設置型 8件 + 架空機本体 16件)
  - 要判断: 0
- MD対応状況:
  - 対応物あり: 25(超兵器7件 + 種別④18件)
  - 該当なし: 0
