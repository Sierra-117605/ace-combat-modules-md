...
- **備考**: アリコーン独自のモジュール候補は **SRC-03a 超大型レールキャノン**
  (600mm、核砲弾発射可)、**SLUAV 発射ベイ**(潜水艦から空中無人機を射出)、
  **潜水艦用艦上滑走路**(20-30機運用、シンファクシ級と共有)の3点。
  全件 MD既存と非競合のため種別④に追加済み。
  200mm 副砲レールガン / 48基VLS は MD既存(`module_railguns_category` /
  `module_vls_category`)と直接競合のため独自モジュール化はせず、MDの既存
  モジュールを最大tierで割り当てる想定。
...
### 種別③ 由来の艦載モジュール候補(MD既存非競合のみ採用、4件)
...
- 種別別件数:
  ...
  - モジュール構成要素: 17(航空機13件 + 艦載4件)
  - 実在機: 0
- 判定別件数:
  - ○ モジュール化可: 17(航空機モジュール13件 + 艦載モジュール4件)
  - △ アーキタイプ追加: 9(超兵器セクションの空中艦・空母系)
  - × 除外: 24(超兵器固定設置型 8件 + 架空機本体 16件)
  - 要判断: 0
- MD対応状況:
  - 対応物あり: 24(超兵器7件 + 種別④17件)
  - 該当なし: 0
