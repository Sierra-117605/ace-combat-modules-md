...
| G | 子機搭載ドローン群 | 2 | 子機ベイ枠 | CFA-44 UAV-45s、ADF-11F 支援UAV |
| G | UAVネットワーク制御 | 1 | アビオ枠 | QFA-44 + MQ-90L Quox bis |
| G | 群運用攻撃UAV(MQ-99系) | 1 | 子機ベイ枠 / MD既存 `weap_droneswarm_fighter_1` 系拡張 | MQ-99 |
...
- 区分 A:1 / B:2 / C:1 / D:3 / E:2 / G:3 / H+I:1 / J:1 = **計14モジュール**
...
| `plane_droneswarm_weapon` | MD_plane_modules.txt:8213〜 | 子機搭載ドローン群、群運用攻撃UAV(MQ-99系) |
...
- **性質**: 親機(母機)に搭載する小型自律ドローン群。空戦中に射出し、
  ロック支援・追撃・攻撃を自律で行う。本MODでは2 tier想定。
- **典拠URL**:
  - https://acecombat.wiki.gg/wiki/CFA-44_Nosferatu
  - https://acecombat.wiki.gg/wiki/ADF-11F_Raven
- **HOI4実装判定**: ○ モジュール化可
...
- **MD既存実装との対応**: 該当カテゴリあり。
  既存定義: `MD_plane_modules.txt:8213〜`(4つの既存 droneswarm_weapon 例あり)。
- **備考**: アーセナルバードのドローンスワーム実装と最も近い参考例。
...
- **性質**: 母機と複数の独立UAV(MQ-90L Quox bis 等)をネットワーク化し、
  母機側からUAV群を統合制御する通信・指揮システム。レーザCIWS のような
  防御運用も可能。
- **典拠URL**:
  - https://acecombat.wiki.gg/wiki/QFA-44_Carmilla
- **HOI4実装判定**: ○ モジュール化可
...
- **備考**: 「子機搭載ドローン群」とは別物(あちらは射出物、こちらは
  独立UAV群との通信・連携)。

---

#### 群運用攻撃UAV (Swarm-deployment Attack UAV / MQ-99 系)

- **種別**: モジュール構成要素
- **登場作品**: AC7(MQ-99、エルジア軍主力UAV)
- **製造主体(in-universe)**: EASA(ベルカ製UAV技術 + Gründer Industries 技術ベース)
- **性質**: コンテナ射出式の戦闘UAV群。母艦/母機から大量射出して群運用。
  9G旋回可能だが脆弱、ミサイル1発で撃墜される使い捨て前提の設計。
- **典拠URL**:
  - https://acecombat.wiki.gg/wiki/MQ-99
- **HOI4実装判定**: ○ モジュール化可
- **判定根拠**: MD既存の `plane_droneswarm_weapon` カテゴリに新モジュール
  として追加。アーセナルバードの `weap_droneswarm_fighter_1` の戦闘機派生として
  位置づける。
- **MD既存実装との対応**: 該当カテゴリあり(`MD_plane_modules.txt:8213〜`)。
- **備考**: 「子機搭載ドローン群」(UAV-45s/ADF-11F支援UAV)が**少数精鋭**の
  支援ドローンであるのに対し、本モジュールは**大量・使い捨て**の攻撃UAV群、
  という運用差で差別化。MD既存のアーセナルバード用ドローンスワームと
  最も近いパターン。
