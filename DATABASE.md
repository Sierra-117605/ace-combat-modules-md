...
(MD β版コードで各モジュールのスロット可否を確認した後に順次追記する)
...
(2026-06-28 再更新: 種別④モジュール構成要素の **候補サマリ14件** を確定。
個別エントリ化はMD β版での裏取り後に順次追加。
現時点では正式エントリ数には含めない)

- エントリ総数: 33(個別エントリのみ。種別④候補サマリ14件は別管理)
- 種別別件数:
  - 超兵器: 17
  - 架空機: 16
  - 架空艦: 0
...
- 判定別件数:
  ...
  - 要判断: 0(MQ-99 と MQ-90 Quox は × 主判定だが種別④で ○ 化検討対象)
- MD対応状況:
  - 対応物あり: 7
  - 該当なし: 0
  - 未確認: 26
