...
**結論先出し(2026-06-28)**: 種別③単独の本体エントリは 0 件で完了。
AC作品の特徴的架空艦は大半が**超兵器分類**で種別①に集約済み(アリコーン、
シンファクシ級、アイガイオン級空中艦、グレイプニル、アーセナルバード、
アークバード)。それ以外の在来型架空艦(Hubert級空母、Leasath 海軍
Proteus/Triton 等)は通常艦扱いで、本MOD方針(艦本体追加は方針外)から外れる。

### 対象艦の振り分け

| 対象艦 | 取り扱い | 参照 |
|--------|----------|------|
| アリコーン (Alicorn) | 超兵器セクション既出(△ アーキタイプ追加) | 種別① |
| シンファクシ級 (Scinfaxi & Hrimfaxi) | 超兵器セクション既出(△ アーキタイプ追加) | 種別① |
| アイガイオン級空中艦(本体・両翼ギュゲス・コットス) | 超兵器セクション既出(△ アーキタイプ追加・mothership_equipment 流用) | 種別① |
| グレイプニル (Gleipnir) | 超兵器セクション既出(△ mothership_equipment 流用) | 種別① |
| アーセナルバード (Arsenal Birds) | 超兵器セクション既出(△ mothership_equipment_1 既実装) | 種別① |
| アークバード (Arkbird) | 超兵器セクション既出(△ mothership_equipment 流用候補) | 種別① |
| Hubert級空母(OFS Kestrel 等) | × 除外(在来型空母、本MOD方針外) | — |
| Leasath 潜水艦 Proteus / Triton | × 除外(在来型潜水艦、本MOD方針外) | — |
| Aegir Fleet | × 除外(通常艦隊、本MOD方針外) | — |

### 艦載兵器(モジュール化候補)

アリコーン搭載の艦載超兵器は **種別④に「艦載モジュール」サブセクション**として
分離し、別途エントリ化する候補がある。MD既存の艦載モジュール枠
(`common/units/equipment/modules/MD_ship_modules.txt`、艦のモジュール定義)への
追加で実装可能。

候補(未エントリ化、後続作業):
- **SRC-03a 大型レールキャノン**(600mm口径、76.5m砲身、核砲弾発射可)
- **200mm 補助レールガン**(80発/分×2基)
- **48基VLS**(戦術/対艦ミサイル、24基×2副船体)
- **16基SLUAV発射ベイ**(無人機 + バリアドローン)
- **艦上滑走路**(20-30機の有人戦闘機運用)

これらは種別④の航空機モジュール14件とは**別系統**(艦載モジュール枠)で
扱う。MD_ship_modules.txt のカテゴリ確認 + ○判定裏取りは次フェーズ
(P3 SPEC精査または種別④第2弾)で行う。

### 統計

- 種別③本体エントリ: 0
- 種別③由来の艦載モジュール候補(未エントリ化): 5
...
- 種別別件数:
  - 超兵器: 17
  - 架空機: 16
  - 架空艦: 0(対象は超兵器側で処理済み or 本MOD方針外。詳細は種別③参照)
  - モジュール構成要素: 13(航空機モジュール。艦載モジュール候補5件は未エントリ)
  - 実在機: 0
- 判定別件数:
  - ○ モジュール化可: 13(全件 種別④モジュール構成要素)
  - △ アーキタイプ追加: 9(全件 超兵器セクションの空中艦・空母系)
  - × 除外: 24(超兵器固定設置型 8件 + 架空機本体 16件)
  - 要判断: 0
- MD対応状況:
  - 対応物あり: 20(超兵器7件 + 種別④13件)
  - 該当なし: 0
